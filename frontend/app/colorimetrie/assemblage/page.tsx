        ...
        {result && quality ? (
          ...
          <div className="space-y-6 sm:space-y-8">
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              {error && (
                <div className="max-w-md mx-auto flex items-start gap-2.5 px-4 py-3 rounded-xl bg-red-50 text-red-700 text-[11px] font-medium leading-relaxed border border-red-100">
                  <AlertCircle className="w-3.5 h-3.5 mt-0.5 shrink-0" />
                  {error}
                </div>
              )}
              ...
              <div className="flex flex-col items-center justify-center pt-2">
                <button
                  type="submit"
                  disabled={loading || (!useDb && !file) || (useDb && selectedLotIds.length === 0)}
                  className="w-full max-w-xs flex items-center justify-center gap-2 py-3.5 bg-brand-primary text-white text-xs font-bold rounded-xl shadow-lg shadow-brand-primary/20 hover:bg-brand-primary/90 transition-all disabled:opacity-40 disabled:shadow-none"
                >
                  {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : <Sparkles className="w-4 h-4" />}
                  {loading ? t("colori.computing") : t("colori.compute")}
                </button>
